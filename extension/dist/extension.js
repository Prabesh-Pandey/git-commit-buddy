...
function activate(context) {
    ...
    const onSave = vscode.workspace.onDidSaveTextDocument((doc) => {
        try {
            ...
            for (const g of globs) {
                if (minimatch(rel, g)) { matched = true; break; }
            }
            if (!matched) return;
            ...
            try {
                ...
                if (check.status === 0) { out.appendLine(`git-autopush: file is ignored — skipping: ${rel}`); return; }
            }
            catch (e) { }

            for (const p of sensitivePatterns) {
                if (minimatch(rel, p)) { out.appendLine(`git-autopush: file matches sensitive pattern '${p}' — skipping: ${rel}`); return; }
            }
            ...
            try {
                ...
                try { branch = require('child_process').execSync('git rev-parse --short HEAD', { cwd: repoRoot, stdio: ['ignore','pipe','ignore'] }).toString().trim(); } catch (e2) { branch = ''; }
            }
            ...
            if (scriptPath.startsWith('~')) scriptPath = path.join(process.env.HOME || '', scriptPath.slice(1));
            scriptPath = scriptPath.replace(/\$\{[^}]+\}/g, '');
            if (!require('path').isAbsolute(scriptPath) && workspaceFolder) scriptPath = path.resolve(workspaceFolder, scriptPath);
            if (!require('fs').existsSync(scriptPath)) {
                const fallback = path.join(workspaceFolder || '', 'git-autopush.sh'); if (require('fs').existsSync(fallback)) scriptPath = fallback;
            }
            ...
            out.appendLine('git-autopush: onSave handler error');
        }
    });
    ...
    const setApiKeyCmd = vscode.commands.registerCommand('git-autopush.setApiKey', async () => {
        try {
            const value = await vscode.window.showInputBox({ prompt: 'Enter AI provider API key (stored machine-local)', ignoreFocusOut: true, password: true });
        ...
        catch (e) { vscode.window.showErrorMessage('git-autopush: failed to save API key.'); }
    });
    ...
    const generateMessageCmd = vscode.commands.registerCommand('git-autopush.generateMessage', async () => {
        ...
        if (!editor) { vscode.window.showWarningMessage('git-autopush: no active editor'); return; }
        ...
        const workspaceFolder = (vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders[0]) ? vscode.workspace.workspaceFolders[0].uri.fsPath : '';
        const rel = workspaceFolder ? path.relative(workspaceFolder, doc.uri.fsPath) : doc.uri.fsPath;
        if (!aiEnabled || !generate) { vscode.window.showInformationMessage('git-autopush: AI commit generation is disabled (enable via settings).'); return; }
        ...
        async function presentSuggested(suggested) {
            if (review) {
                ...
                if (typeof edited === 'string') { context.workspaceState.update('gitAutopush.lastAIMessage', edited); vscode.window.showInformationMessage('git-autopush: commit message ready. Use Run Once to execute.'); }
                else { vscode.window.showInformationMessage('git-autopush: message review cancelled.'); }
            }
            else { context.workspaceState.update('gitAutopush.lastAIMessage', suggested); vscode.window.showInformationMessage('git-autopush: AI message generated and stored (use Run Once to apply).'); }
            updateStatusBar();
        }
        ...
        if (provider === 'openai' && apiKey && apiKey.length > 0) {
            ...
            try {
                ...
                const prompt = `You are a concise commit message generator. Summarize the primary change(s) in one short imperative sentence (max 72 chars). Filename: ${path.basename(rel)}\n\n${bodyText}`;
                const postData = JSON.stringify({ model: 'gpt-3.5-turbo', messages: [{ role: 'system', content: 'You generate short, clear git commit messages.' }, { role: 'user', content: prompt }], max_tokens: 60, temperature: 0.2 });
                const reqOpts = { hostname: 'api.openai.com', port: 443, path: '/v1/chat/completions', method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(postData), 'Authorization': `Bearer ${apiKey}` } };
                    ...
                    const req = https.request(reqOpts, (res) => {
                        ...
                        res.on('end', () => {
                            try {
                                ...
                                if (text && typeof text === 'string') resolve(text.trim()); else reject(new Error('No message in AI response'));
                            }
                            catch (e) { reject(e); }
                        });
                    });
            ...
            catch (e) { out.appendLine('git-autopush: AI provider call failed — falling back.'); }
        }
        ...
        if (provider === 'deepseek' && apiKey && apiKey.length > 0) {
            ...
            try {
                ...
                const reqOpts = { hostname: urlObj.hostname, port: urlObj.port || 443, path: urlObj.pathname + (urlObj.search || ''), method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(payload), 'Authorization': `Bearer ${apiKey}` } };
                const suggested = await new Promise((resolve, reject) => {
                    const req = https.request(reqOpts, (res) => {
                        let data = '';
                        res.on('data', (chunk) => data += chunk);
                        res.on('end', () => {
                            try {
                                ...
                                if (text && typeof text === 'string') resolve(text.trim()); else reject(new Error('No message in DeepSeek response'));
                            }
                            catch (e) { reject(e); }
                        });
                    });
            ...
            catch (e) { out.appendLine('git-autopush: DeepSeek provider call failed — falling back.'); }
        }
    ...
    const runOnceCmd = vscode.commands.registerCommand('git-autopush.runOnce', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) { vscode.window.showWarningMessage('git-autopush: no active editor to run on'); return; }
        const doc = editor.document;
        const cfg = vscode.workspace.getConfiguration('gitAutopush');
        ...
        scriptPath = scriptPath.replace(/\$\{[^}]+\}/g, '');
        if (!require('path').isAbsolute(scriptPath) && workspaceFolder) scriptPath = path.resolve(workspaceFolder, scriptPath);
        if (!require('fs').existsSync(scriptPath)) { const fallback = path.join(workspaceFolder || '', 'git-autopush.sh'); if (require('fs').existsSync(fallback)) scriptPath = fallback; }
